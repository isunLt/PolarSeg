...
from dataloader.dataset import collate_fn_BEV, SemKITTI, SemKITTI_label_name, spherical_dataset, voxel_dataset, Xentropy_loss_w
...
def main(args):
    ...
    compression_model = args.grid_size[2]  # default 32
    ...
    loss_fun = torch.nn.CrossEntropyLoss(weight=Xentropy_loss_w.to(pytorch_device), ignore_index=255)
    # loss_fun = torch.nn.CrossEntropyLoss(ignore_index=255)

    # prepare dataset
    train_pt_dataset = SemKITTI(data_path + '/sequences/', imageset='train', return_ref=True)  # ([x,y,z], [label], [i])
    ...
    while epoch < 30:
        ...
        for i_iter, (_, train_vox_label, train_grid, _, train_pt_fea) in enumerate(train_dataset_loader):
                ...
                with torch.no_grad():
                    for i_iter_val, (_, val_vox_label, val_grid, val_pt_labs, val_pt_fea) in enumerate(val_dataset_loader):
                        ...
                        loss = lovasz_softmax(torch.nn.functional.softmax(predict_labels).detach(), val_label_tensor,
                                              ignore=255) + loss_fun(predict_labels.detach(), val_label_tensor)  # 255->0
                        predict_labels = torch.argmax(predict_labels, dim=1)
                        predict_labels = predict_labels.cpu().detach().numpy()  # [val_batch_size, 480, 360, 32]
                        for count, i_val_grid in enumerate(val_grid):
                            # val_grid中为点云中每个点对应的格子序号
                            # predict_labels为每个格子被预测的label
                            # 因此每个点label就等于其对应格子的label，由于多个点可能对应同一个格子，所以预测值会存在系统性误差
                            hist_list.append(fast_hist_crop(predict_labels[count, val_grid[count][:, 0], val_grid[count][:, 1], val_grid[count][:, 2]],
                                                            val_pt_labs[count],
                                                            unique_label))
            ...
            try:
                ...
                # print("unique train_vox_label: ", np.unique(train_vox_label.flatten()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--grid_size', nargs='+', type=int, default=[360, 270, 32],
